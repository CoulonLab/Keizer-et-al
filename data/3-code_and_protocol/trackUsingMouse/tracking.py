...
def reorderImgDim(im,source,target):
    """e.g. reorderImgDim(im, source='TZCYX', target='TCXYZ')"""
    if len(im.shape)!=len(source):
        raise ValueError('`im` should have as many dimensions as elements in `source`')
    target=''.join([s for s in target.upper() if s in source.upper()])
    
    return moveaxis(im,range(len(im.shape)),
                   [target.index(source[i].upper()) for i in range(len(source))])

def checkImgDim(fnTif):
  print('The dimensions of your image are:',io.imread(fnTif).shape); 
...
def bpass(im,r1=1.,r2=1.7):
  ...
  ker1x=exp(-(x/r1)**2/2); ker1x/=sum(ker1x); fker1x=fft.fft(ker1x);
  ker1y=exp(-(y/r1)**2/2); ker1y/=sum(ker1y); fker1y=fft.fft(ker1y);
  ker2x=exp(-(x/r2)**2/2); ker2x/=sum(ker2x); fker2x=fft.fft(ker2x);
  ker2y=exp(-(y/r2)**2/2); ker2y/=sum(ker2y); fker2y=fft.fft(ker2y);
  fim=fftpack.fftn(im)
...
def trackUsingMouse(fnTif,fnSuff,
                    ...
                    channel=None,
                    performFit=True,
                    psfPx=1.7, psfZPx=None,
                    thresholdSD=5,
                    roiSize=40, border=3, 
                    distThreshold=20, trackMemory=0, reactionDelay=.0,
                    inputImgDim=None,
                    disp=3):
  ...
  - channel:       This indicates which channel of a multi-channel image is
                   selected when Fiji opens. it can be ommitted. In all cases, the
                   channel used for spot detection and fitting is the one on which
                   the Fiji window is when performing the manual tracking.
                   0-based. (Default: None)
  ...
  - inputImgDim:   Dimensions of input images. Run checkImgDim().
                   (Default: None, which is interprested as T[Z][C]YX).
...
if (channels==1) {
    ...
    if (slices==1) { print('# T C X Y'); }
    else           { print('# T C X Y Z'); } }
...
while (isOpen(title)) {
  ...
  if (newImgNb!=currImgNb) {
    ...
    if (channels==1) {
        ...
        if (slices==1) { print(currFrame, currChannel, x, y); }
        else           { print(currFrame, currChannel, x, y, currSlice); } }
...
selectWindow('Log');
saveAs('Text','%s');"""%(os.path.basename(fnTif),
                         fnTif,
                         os.path.basename(fnTif),
                         "" if channel is None else "Stack.setChannel(%d)"%(channel+1),
                         fnTif[:-4]+fnSuff+'.txt')
  ...
  lPosGuess=loadtxt(fn_mtrk,skiprows=1) # Columns should be T[C]XY[Z]
  lPosGuess_header=open(fn_mtrk,'r').readline().replace('\n','')
  ...
  if fps==0:
    ...
    lPosGuess=tmp
    savetxt(fn_mtrk,lPosGuess,header=lPosGuess_header,fmt="%d")
...
#    lPosGuess=loadtxt(fnTif[:-4]+fnSuff+'.mtrk',skiprows=1)
  ...
  a=io.imread(fnTif); 
  #print(a.shape)
  if inputImgDim is not None:
    reorderImgDim(a, source=inputImgDim, target='TCXYZ') 
  else: ## Assuming T[Z][C]YX, i.e. imagej's default
      a=a.swapaxes(-1,-2) # => T[Z][C]XY
      if 'Z' in lPosGuess_header:
        a=rollaxis(a,1,4) # => T[C]XY[Z]
  # `a` should now be T[C]XY[Z], i.e. same as the columns in `lPosGuess`
  ...
  ####################################################
  if 'Z' not in lPosGuess_header: ########## Data set is 2D #
        
      for i in range(nbFrames):
        if 'C' in lPosGuess_header:
          posGuess=lPosGuess[max(0,i-frameDelay)][2:] # XY
          im1=a[i,int(lPosGuess[i,1])-1]
        else:
          posGuess=lPosGuess[max(0,i-frameDelay)][1:] # XY
          im1=a[i]
        box=c_[(posGuess[0]+r_[-1,1]*(roiSize//2+border)).clip(0,im1.shape[1]-1),
               (posGuess[1]+r_[-1,1]*(roiSize//2+border)).clip(0,im1.shape[0]-1)].flatten().astype(int)
        im2=im1[:,box[0]:box[2]][box[1]:box[3]]                   # Cropped image
      ...
      lPos=c_[lPosGuess[maximum(0,r_[:nbFrames]-frameDelay)][:,-2:],zeros(nbFrames)]
      ...
      for i in range(nbFrames):
        ...
        pt=(posGuess[-2:]-box[:2]-border+.5).astype(int);
      ...
      print("Frames with successful fit: %.1f%% (%d/%d)"%(nbConv/len(trk2)*100,nbConv,len(trk2)))
      ...
      if disp&1:
        ...
        fig.add_subplot(411); plt.grid(1); plt.ylabel('X coordinate (px)'); plt.plot(lPosGuess[:,0],lPosGuess[:,-2],label="Manual track"); plt.plot(trk2[:,0],trk2[:,1],label="Gaussian fit"); plt.legend()
        fig.add_subplot(412); plt.grid(1); plt.ylabel('Y coordinate (px)'); plt.plot(lPosGuess[:,0],lPosGuess[:,-1],label="Manual track"); plt.plot(trk2[:,0],trk2[:,2],label="Gaussian fit"); plt.legend()
  ...
  ####################################################
      ...
      lPosGuess[:,-1]-=1 # Convert z to zero-based indexing
    
      for i in range(nbFrames):
        if 'C' in lPosGuess_header:
          posGuess=lPosGuess[max(0,i-frameDelay)][2:] # XYZ
          im1=a[i,:,int(lPosGuess[i,1])-1]
        else:
          posGuess=lPosGuess[max(0,i-frameDelay)][1:] # XYZ
          im1=a[i]
        box=c_[(posGuess[0]+r_[-1,1]*(roiSize//2+border)).clip(0,im1.shape[2]-1),
               (posGuess[1]+r_[-1,1]*(roiSize//2+border)).clip(0,im1.shape[1]-1)].flatten().astype(int)
        #print(posGuess,im1.shape,box)
        im2=im1[:,:,box[0]:box[2]][:,box[1]:box[3]]               # Cropped image
        ...
        res.append([box,im2])
        #print(posGuess,im2.shape,im3.shape,im1.shape,int(posGuess[2]))
      ...
      lPos=c_[lPosGuess[maximum(0,r_[:nbFrames]-frameDelay)][:,-3:],zeros(nbFrames)]
      for rp in resPos: lPos[int(rp[0])]=rp[1:]
      ...
      for i in range(nbFrames):
        ...
        im8=toDisp[i][0]*0; # Image with 0.3 = mouse position, 0.7 = bpass object, 1 = fit (if converged)
        pt=(posGuess[-3:-1]-box[:2]-border+.5).astype(int);
        if prod(0<=pt)*prod(pt<roiSize): im8[pt[1],pt[0]]=.3
        pt=(pos[:2]-box[:2]-border+.5).astype(int);
        if prod(0<=pt)*prod(pt<roiSize): im8[pt[1],pt[0]]=.7
        if conv:
      ...
      if disp&1:
        ...
        fig.add_subplot(511); plt.grid(1); plt.ylabel('X coordinate (px)'); plt.plot(lPosGuess[:,0],lPosGuess[:,-3],label="Manual track"); plt.plot(trk2[:,0],trk2[:,1],label="Gaussian fit"); plt.legend()
        ...
        fig.add_subplot(513); plt.grid(1); plt.ylabel('Z coordinate (px)'); plt.plot(lPosGuess[:,0],lPosGuess[:,-1],label="Manual track"); plt.plot(trk2[:,0],trk2[:,3],label="Gaussian fit"); plt.legend()
